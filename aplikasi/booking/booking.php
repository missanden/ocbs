...
switch ($_GET['act']) {
    ...
    case "menubooking":
        ...
        if ($_SESSION['s_position'] == 'HOD') {
            $sql = "SELECT * FROM master_mobil 
                WHERE 
                    kepemilikan_mobil IN ('operasional','$_SESSION[s_no_employee]')
                AND 
                    aktif_mobil IN ('aktif') 
                ORDER BY 
                    kepemilikan_mobil DESC";
        } else {
            $sql = "SELECT * FROM master_mobil 
                    WHERE 
                        kepemilikan_mobil IN ('operasional') 
                    AND 
                        aktif_mobil IN ('aktif')";
        }
    ...
    case "bookingmobil":
        if (isset($_POST['submit'])) {
            $now = date('Y-m-d');
            $queryvalid = "SELECT * FROM booking_mobil 
                            INNER JOIN
                                booking_selesai ON booking_mobil.kode_booking=booking_selesai.kode_booking
                            WHERE
                                booking_mobil.no_polisi='$_POST[no_polisi]' AND
            ...
            IF ($row == '0') {
				...
				'booked')";
                    ...
                    for ($awal; $awal <= $akhir; $awal++) {
                        ...
                        $sql = "INSERT INTO schedule_booking
                                    (kodeunik,
                                    kode_booking) 
                                VALUES 
                                    ('$kodeunik',
                ...
                $sql2 = "INSERT INTO booking_selesai
                            (kode_booking,
                            jam_selesai) 
                        VALUES 
                            ('$kode_booking',
                 ...
                 */
                if ($_SESSION['s_position'] == 'HOD') {
                    $view = mysql_query("SELECT * FROM master_employee 
                                        INNER JOIN 
                                            section ON master_employee.kode_section=section.kode_section
					INNER JOIN 
                                            department ON department.kode_department=section.kode_department
                                        WHERE 
                                            department.nama_department LIKE '%$_SESSION[s_department]%' 
                                        AND 
                                            master_employee.position_user LIKE '%presdir%'");
                    ...
                    $booking = mysql_query("SELECT * FROM booking_mobil 
						...
						master_employee ON booking_mobil.no_employee=master_employee.no_employee
                                            ...
                                            INNER JOIN 
                                                department ON department.kode_department=section.kode_department
                                            WHERE 
                                                booking_mobil.kode_booking='$kode_booking' 
                                            ORDER BY 
                                                booking_mobil.jam_start ASC");
...
//================================================mail for hod=========================================				
                    $view = mysql_query("SELECT * FROM master_employee 
                                        ...
                                        WHERE 
                                            department.nama_department LIKE '%$_SESSION[s_department]%' 
                                        AND 
                                            master_employee.position_user LIKE '%HOD%'");
                    $r = mysql_fetch_array($view);
...
//===============================================================================	
        ...
        while ($d = mysql_fetch_array($query)) {
            ...
            $tampil = mysql_query("SELECT * FROM jam_booking 
                                ORDER BY 
                                    jam_start ASC");
            while ($w = mysql_fetch_array($tampil)) {
    ...
    case "bookingmobil2":
        ...
        if (isset($_POST['submit'])) {
            ...
            $queryjamstart = "SELECT * FROM jam_booking 
                            WHERE
                                jam_booking.kode_jam='$_POST[jam_start]'";
            ...
            $queryjamselesai = "SELECT * FROM jam_booking 
                                WHERE 
                                    kode_jam='$jamselesaii'";
            ...
            IF ($row == '0') {
                ...
                for ($kode_jam1; $kode_jam1 <= $kode_jam2; $kode_jam1++) {
                    ...
                    $sql2 = "INSERT INTO schedule_booking
                                (kodeunik,
                                kode_booking) 
                            VALUES 
                                ('$kodeunik',
...
//================================================mail for hod=========================================				
                    ...
                    IF ($_SESSION['s_department'] == 'Quality Assurance') {
                        ...
                        $spvmail = "tommy@sanden.co.id";
                        $ccmail = "anriga@sanden.co.id";
                    } ELSE IF ($_SESSION['s_department'] == 'Marketing & Sales') {
                        $spvmail = "andreansyah@sanden.co.id";
                    } else {
                        $spvmail = $r['personal_email_employee'];
                    }
                    $booking = mysql_query("SELECT * FROM booking_mobil 
        ...
        while ($d = mysql_fetch_array($query)) {
                    ...
                    <table cellpadding='2' cellspacing='0' border='0'>
                        <tr>
                            ...
                            <td valign='top'>
                                <div class='section'>Detail Booking</div>
                                <div class='inner-wrap'>
                                    <label>Tanggal Booking <br> $_POST[tanggal] <input type='hidden' size='35' name='tanggal_pemakaian' value='$_POST[tanggal]'  id='tanggal1' size=10 ></label>
                                    <label>Jam Mulai <select name='jam_start' id='jammulai' width='20'>
                                    <option value=''>Jam Mulai</option>";
            ...
            $tampil2 = mysql_query("SELECT * FROM master_employee WHERE grade_employee NOT LIKE '8%' AND grade_employee NOT LIKE '9%' AND grade_employee NOT LIKE '9%' AND no_employee NOT IN('$_SESSION[s_no_employee]') ORDER BY nama_employee ASC");
            while ($w = mysql_fetch_array($tampil2)) {
                echo "<option value='$w[nama_employee]'>$w[nama_employee]</option>";
            }
            echo "
    ...
    case "bookingrental":
        if (isset($_POST['submit'])) {
            ...
            IF ($row == '0') {
                ...
                //================================================mail for hod=========================================		
                    $view5 = mysql_query("SELECT * FROM master_employee 
                                        INNER JOIN 
                                            section ON master_employee.kode_section=section.kode_section
					INNER JOIN 
                                            department ON department.kode_department=section.kode_department
                                        WHERE 
                                            department.nama_department LIKE '%$_POST[no_employee]%'");
					...
					INNER JOIN 
                                            department ON department.kode_department=section.kode_department
                                        WHERE 
                                            department.nama_department LIKE '%$namadepartment%' AND 
                                            master_employee.position_user LIKE '%HOD%'");
                    $r = mysql_fetch_array($view);
...
<input type='submit' name='cancel' id='cancel' value='Hapus'></form>";
    ...
    case "approvalbooking":
        ...
        if ($row == '0') {
            echo "<br><br><br><br><br><center>NO DATA APPROVAL</center><br><br><br><br><br><br>";
        } else {
...
//$headers .= 'Bcc: dodi.kesumayadi@sanden.co.id' . "\r\n";
        ...
        while ($d = mysql_fetch_array($query)) {
            ...
            if ($d['approved'] == '0') {
                echo "<input type='text' size='35' disabled name='instansi' value='Proses Persetujuan HOD' size=10>";
            } else if ($d['approved'] == '1') {
                echo "<input type='text' size='35' disabled name='instansi' value='Disetujui HOD' size=10>";
            } else if ($d['approved'] == '2') {
                echo "<input type='text' size='35' disabled name='instansi' value='Tidak Disetujui HOD' size=10>";
            } else {
                echo "<input type='text' size='35' disabled name='instansi' value='Disetujui HOD' size=10>";
            }
            echo "</div>
